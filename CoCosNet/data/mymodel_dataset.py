...
class MyModelDataset(Pix2pixDataset):
  ...
  def get_paths(self, opt):
    ...
    self.image_paths = ['./imgs/mymodel/source_img/00001.jpg','./imgs/mymodel/source_img/00002.jpg',
                   './imgs/mymodel/source_img/00003.jpg', './imgs/mymodel/source_img/00004.jpg']
    self.label_paths = ['./imgs/mymodel/source_img/00001.jpg','./imgs/mymodel/source_img/00002.jpg',
                   './imgs/mymodel/source_img/00003.jpg', './imgs/mymodel/source_img/00004.jpg']
    ...
    return self.label_paths, self.image_paths

  def get_ref(self, opt):
    extra = ''
    if opt.phase == 'test':
      extra = '_test'
    with open('./data/mymodel_ref{}.txt'.format(extra)) as fd:
      lines = fd.readlines()
    ref_dict = {}
    for i in range(len(lines)):
  ...
  def get_label_tensor(self, path):
    inner_parts = ['skin', 'l_brow', 'r_brow', 'l_eye', 'r_eye', 'l_ear', 'r_ear', 'nose', 'u_lip', 'mouth', 'l_lip',
                   'eye_g', 'hair']
    ...
    canny_edges = cv2.Canny(np.array(img.convert('L')),80,255)
    ...
    label_tensor = tensor_edges_all
    tensors_dist = torch.zeros([13,256,256])
    label_tensor = torch.cat((tensor_edges_all, tensors_dist, tensor_edges), dim=0)
    tmp = label_tensor.numpy()
