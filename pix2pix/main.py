...
def main(config):
    ...
    for epoch in range(config.epoch, config.n_epochs):
        for i, (im1, m1, im2, m2) in enumerate(train_loader):
            ...
            # GAN loss
            z1 = resnet(im1 * m1) #skin
            z2 = resnet(im2 * m2) #hair
            fake_im = generator(im1, z1, z2) # z1 is skin, z2 is hair

            # pred_fake = discriminator(fake_im, im2)
            # loss_GAN = config.lambda_gan * criterion_GAN(pred_fake, valid)

            # Hair, Face loss
            fake_m2 = torch.argmax(hair_model(fake_im), 1).unsqueeze(1).type(torch.uint8).repeat(1, 3, 1, 1).to(config.device)
            ...
            if i % config.sample_interval == 0:
                ...
                fname = os.path.join(config.save_path, "Train_Epoch_%d_Batch_%d.png" % (epoch, i))
                ...
                for j, (im1, m1, im2, m2) in enumerate(val_loader):
                    with torch.no_grad():
                        ...
                        msg = "Validation || hair loss: %.6f, face loss: %.6f, loss: %.6f\n" % \
                                (hair_loss.item(), face_loss.item(), loss.item())
                        sys.stdout.write(msg)
                        fname = os.path.join(config.save_path, "Validation_Epoch_%d_Batch_%d.png" % (epoch, i))
                        sample_images([im1[0], im2[0], fake_im[0]], ["img1", "img2", "img1+img2"], fname)
                        break
        ...
        if epoch % config.checkpoint_interval == 0:
            ...
            fnames = [os.path.join(config.checkpoints, 'epoch_%d_%s.pth' % (epoch, s)) for s in fnames]
